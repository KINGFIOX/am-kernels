...
void check_hit(char ch) {
  int m = -1;
  for (int i = 0; i < LENGTH(chars); i++) {
    const struct character *c = &chars[i];
    if (ch == c->ch && c->v > 0 && (m < 0 || c->y > chars[m].y)) { m = i; } // hit!
  }
  if (m == -1) { // statistics
    wrong++;
  } else {
...
void video_init() {
  ...
  for (int i = 0; i < screen_w; i++) { blank_line[i] = COL_PURPLE; }
  for (int y = 0; y < screen_h; y ++) { io_write(AM_GPU_FBDRAW, 0, y, blank_line, screen_w, 1, false); }
  for (int ch = 0; ch < 26; ch++) { // foreach char
    char *c = &font[CHAR_H * ch];
    for (int i = 0, y = 0; y < CHAR_H; y++) { // foreach line
      for (int x = 0; x < CHAR_W; x++, i++) { // foreach pixel
...
int main() {
  ...
  int current = 0, rendered = 0; // 时间戳(单位: 帧)
  const uint64_t t0 = io_read(AM_TIMER_UPTIME).us;
  while (1) {
    const int frames = (io_read(AM_TIMER_UPTIME).us - t0) / (1000000 / FPS); // 时间戳(单位: 帧)
    for (; current < frames; current++) { game_logic_update(current); }
    while (1) {
